...
## 修改内容 2018年01月26日00:56:17
> 1. 修改页面跳转显示不一致等问题；
>
> 2. 修改（骞昊）设计的通过数据逻辑；@骞昊 需要再check一下这部分的逻辑!!!
>> * 该页面是作为后期用户购买数据的页面；
>> * 页面需显示通过的数据资源，并且当前的数据资源不是当前用户所上传的；
>
> 3. 完善（朱峰）设计的数据上传逻辑；@朱峰  需要重点学习一下修改后的逻辑!!!
>> * 对照着model里面的data表，完善了用户上传数据时的信息；
>> * 添加了用户上传数据进行信用coin的奖励，生产新的coin,保存到coin表； 默认奖励coin大小=1
>> * 添加了tx文件的生成(transaction)，当前的action = 'upload'
>> * 用户上传数据完成后，更新用户的wallet; (并且添加了从coin表中读取进行double-check的逻辑)
...
## 2018年01月26日 TODO:
#### 重新拉取分支，命名规则： 时间_姓名_功能
> （骞昊）1.修改购买数据页面，添加购买按钮, 基本实现数据购买功能;
> * 所涉及到的表：
>> * wallet(更新seller 和 buyer的账户余额)，
>> * transaction(生成交易记录)，
>> * urchase(这里的purchase表示购买，生成购买记录，用于表示用户有权限下载此数据),
>> * data表(需要更新数据购买量)
> * 购买时coin来源和输出， 郑鹏飞负责
> * tx(transaction)文件生成规则， 郑鹏飞负责

> （朱峰）1.在用户上传数据时： 需要弹窗提醒用户数据上传成功，跳转页面：用户历史上传的所有数据页面；
> （朱峰）2.设计充值页面，目前充值先手动填写数值
> * 所涉及到的表：
>> * wallet表，当前用户的余额更新；
>> * coin表，需要生成新的coin, 初始化次coin归属与user_Id, 和当前状态为 没有被花费 is_spent = False
> * tx(transaction)文件生成规则， 郑鹏飞负责
...
| 数据id | 数据拥有者id| 数据名| 数据简介信息 | 数据上传时间 | 数据来源 |数据保存格式 |数据tag |数据审核状态|数据md5值|数据大小|数据下载量|数据购买量|数据定价|数据保存服务器地址|
| ----- |:-------------:| :------:|:-------:| :----:| :----:| :----:| :----:|:----:|:----:|:----:|:----:| :----:|:----:|:----:|
