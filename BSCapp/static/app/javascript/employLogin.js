...
function Func_validate() {
  ...
  $.ajax({
    ...
    if(data.statCode != 0) {
     ...
     $.cookie('username', data.username, {path: '/'})
     $.cookie('password', md5(pwd), {path: '/'})
     if ((data.username[0]=='R')|(data.username[0]=='r'))
        indexURL = "/receiverInfo/"
     else if ((data.username[0]=='L')|(data.username[0]=='l'))
        indexURL = "/librarianInfo/"
     else if ((data.username[0]=='D')|(data.username[0]=='d'))
        indexURL = "/distributorInfo/"
